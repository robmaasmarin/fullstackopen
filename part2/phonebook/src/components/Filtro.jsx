...
const Filtro = ({ persons }) => {
  ...
  const filterContacts = persons.filter((f) =>
    f.name.toLowerCase().includes(searching.toLowerCase())
  );
  return (
    <div>
    <p>filter shown with{" "}</p>
      ...
      <ul>
        ...
        (filterContacts.map((contacto , i) => <li key={i}>{contacto.name}{" "}{contacto.number}</li>)
        ) : (<li>No results</li>)  
      }
      </ul>
